...
async function fetchFavorites() {
  ...
  do {
    ...
    const { items, nextToken: newToken } = result.data.listFavorites;
...
async function getShowsWithFavorites(favorites = []) {
  ...
  try {
    ...
    const favoritesList = favorites.length > 0 ? favorites : (await fetchFavorites()).map((f) => f.cid);

    const showsWithFavorites = shows.map((show) => ({
      ...show,
      isFavorite: favoritesList.includes(show.id),
    }));
