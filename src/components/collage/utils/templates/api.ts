...
import type {
  ...
  GetTemplateQuery,
  TemplatesByOwnerIdentityIdQuery,
...
async function uploadSnapshot(
...
): Promise<{ snapshotKey: string; snapshotVersion: number }> {
  ...
  const cached = getCachedProjectRaw(id);
...
export async function loadProjects({ forceRefresh = false }: { forceRefresh?: boolean } = {}): Promise<CollageProject[]> {
  ...
  if (forceRefresh) {
    clearCacheState({ notify: false });
    setCacheComplete(false);
  }

  if (isCacheComplete() && !forceRefresh) {
    return cacheToArray();
  }
...
export async function upsertProject(id: string, payload: UpsertProjectPayload): Promise<CollageProject> {
  ...
  const current = getCachedProjectRaw(id) || (await fetchTemplateById(id));
  if (!current) {
...
export async function resolveThumbnailUrl(
  ...
  if (project.thumbnailKey) {
    ...
    const cached = getThumbnailCacheEntry(project.thumbnailKey);
    const now = Date.now();
    ...
    try {
      ...
      if (typeof url === 'string') {
        setThumbnailCacheEntry(project.thumbnailKey, {
          url,
          signature,
...
export async function resolveTemplateSnapshot(
  ...
  const ensureRecord = async (): Promise<CollageProject | null> => {
    if (typeof projectOrId === 'string') {
      if (hasCachedProject(projectOrId)) return getCachedProjectRaw(projectOrId);
      return fetchTemplateById(projectOrId);
    }
    if ('name' in projectOrId) {
      return projectOrId as CollageProject;
    }
    const cached = getCachedProjectRaw(projectOrId.id);
