...
const ProPage = () => {
	...
	const dest = useMemo(() => destParam || '/', [destParam]);
	...
	}, [user?.userDetails?.subscriptionStatus, dest, navigate]);
	...
	const handleLogin = () => {
		const proWithDest = `/pro${dest ? `?dest=${encodeURIComponent(dest)}` : ''}`;
	...
	const titleSubtitlePairs = [
		{ title: 'Get memeSRC Pro!', subtitle: "Or don't. I don't care." },
		{ title: 'Get Pro. Be a Hero.', subtitle: 'Or stay basic I guess. Your choice.' },
		{ title: 'Unlock memeSRC Pro!', subtitle: 'Or forget you ever saw this.' },
		{ title: 'Pro is for pros.', subtitle: "But don't let that stop you." },
	];
		...
		<Box sx={{ maxWidth: 1100, mx: 'auto', my: 2, px: { xs: 1, sm: 2 } }}>
			<Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', py: isCompact ? 1.2 : 2 }}>
				...
				<IconButton
					...
					size="small"
					aria-label="close"
					sx={{ position: 'absolute', top: isCompact ? 4 : 8, right: 10, zIndex: 1000, opacity: 0.4 }}
				>
					<CloseIcon />
				</IconButton>
			</Box>
			...
			{!loading && !checkoutLink && (
				<Fade in timeout={400}>
					<Box sx={{ py: isCompact ? 2 : 4, pb: isCompact ? 3 : 6 }}>
						...
						<Grid container spacing={2}>
							<Grid item xs={12} sm={CURRENT_SALE.isActive ? (isCompact ? 12 : 6) : 12}>
								<Box
									p={isCompact ? 2 : 2.5}
									sx={{
										backgroundColor: CURRENT_SALE.isActive ? 'rgba(0, 0, 0, 0.2)' : getColor(),
										borderRadius: 4,
										mb: 2,
										cursor: 'pointer',
										position: 'relative',
								...
								>
									<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
										<Typography fontSize={isCompact ? 18 : 22} fontWeight={700} color={CURRENT_SALE.isActive ? 'common.white' : getTextColor()}>
											{selectedTitleSubtitle?.title}
										</Typography>
										{CURRENT_SALE.isActive && (
											<Chip label={`${(CURRENT_SALE.discountPercent).toFixed(0)}% OFF`} color="error" size="small" sx={{ fontWeight: 700, fontSize: '0.85rem', backgroundColor: '#ff1744' }} />
										)}
									</Box>
									<Typography variant={isCompact ? 'h2' : 'h3'} mb={0.75} sx={{ color: CURRENT_SALE.isActive ? getColor() : getTextColor() }}>
										{CURRENT_SALE.isActive && (
									...
									<Typography fontSize={isCompact ? 15 : 13} fontWeight={600} color={CURRENT_SALE.isActive ? 'common.white' : getTextColor()}>
						...
						<Box mt={creditOptionsExpanded || isCompact ? 2 : 4} textAlign="center">
							{user?.userDetails ? (
								...
								}} fullWidth sx={{ borderRadius: 50, px: 4, py: isCompact ? 1.25 : 1.5, fontSize: isCompact ? 18 : 20, backgroundColor: getColor(), color: getTextColor() }}>
									Upgrade Account
								</Button>
							) : (
								<Button ref={subscribeButtonRef} variant="contained" size="large" onClick={handleLogin} fullWidth sx={{ borderRadius: 50, px: 4, py: isCompact ? 1.25 : 1.5, fontSize: isCompact ? 18 : 20, backgroundColor: getColor(), color: getTextColor() }}>
									Upgrade Account
								</Button>
							)}
							...
							{dest && (
								<Button onClick={closeAndReturn} sx={{ mt: 2, color: 'white', opacity: 0.7, '&:hover': { opacity: 1, backgroundColor: 'rgba(255, 255, 255, 0.05)' } }} startIcon={<ArrowBackIcon />}>
									Cancel and go back
								</Button>
							)}
						</Box>
			...
			{(loading || checkoutLink) && (
				<>
					{countryCode !== 'US' && countryCode !== 'AU' && countryCode !== 'CA' && (
						<Box sx={{ py: 4 }}>
							<Box p={3} sx={{ backgroundColor: 'error.main', borderRadius: 4, mb: 4 }}>
							...
							<Box sx={{ mt: 4, mx: 2 }}>
								...
								<Box sx={{ pl: 4, mt: 2 }}>
									<Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
										<Box sx={{ backgroundColor: 'primary.main', borderRadius: '50%', width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', mr: 2 }}>
							...
							<Button variant="contained" size="small" onClick={closeAndReturn} fullWidth sx={{ borderRadius: 50, px: 4, fontSize: 17, mt: 5 }}>
								Dismiss
							</Button>
						</Box>
					)}
					{(countryCode === 'US' || countryCode === 'AU' || countryCode === 'CA') && (
						<Box sx={{ minHeight: 500, display: 'flex', flexDirection: 'column', mb: 5 }}>
							<Button onClick={() => { setCheckoutLink(undefined); setLoading(false); }} sx={{ color: 'white', opacity: 0.7, alignSelf: 'flex-start', mb: 2, '&:hover': { opacity: 1, backgroundColor: 'rgba(255, 255, 255, 0.1)' } }} startIcon={<ArrowBackIcon />}>Go Back</Button>
							<Box sx={{ m: 'auto' }}>
								<Typography fontSize={20} textAlign='center' fontWeight={700}>Powered by</Typography>
								<Typography fontSize={45} textAlign='center' fontWeight={700} pt={0.5}>Vibe House</Typography>
								<Box sx={{ mt: 3, mb: 3, mx: 'auto', p: 2.5, maxWidth: 360, border: '1px solid rgba(255, 255, 255, 0.15)', borderRadius: 2.5, backgroundColor: 'rgba(255, 255, 255, 0.05)', backdropFilter: 'blur(10px)', boxShadow: '0 4px 24px rgba(0, 0, 0, 0.15)' }}>
									<Typography fontSize={13} color="text.secondary" sx={{ mb: 1.5, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 1 }}>
										<Typography component="span" sx={{ position: 'absolute', top: 0, left: 24, transform: 'translateY(-50%)', backgroundColor: 'background.paper', px: 1, fontSize: 13, fontWeight: 600, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: '0.5px', display: 'flex', alignItems: 'center', gap: 1 }}>
											<Box component="span" sx={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: 'success.main', animation: 'pulse 2s infinite', '@keyframes pulse': { '0%': { opacity: 0.4 }, '50%': { opacity: 1 }, '100%': { opacity: 0.4 } } }} />
											<b>Reminder</b>
										</Typography>
										Your subscription to memeSRC Pro will be billed monthly and appear on your statement as:
									</Typography>
									<Box sx={{ p: 1.5, backgroundColor: 'rgba(0, 0, 0, 0.25)', borderRadius: 1.5, border: '1px solid rgba(255, 255, 255, 0.12)', position: 'relative', overflow: 'hidden' }}>
										<Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'relative', zIndex: 1 }}>
											<Typography fontFamily="'Roboto Mono', monospace" fontSize={{ xs: 13, sm: 16 }} fontWeight={700} sx={{ letterSpacing: '0.5px', mx: 1 }}>
												${getPriceForPlan(selectedPlan).toFixed(2)}
											</Typography>
											<Typography fontFamily="'Roboto Mono', monospace" fontSize={{ xs: 13, sm: 16 }} fontWeight={700} sx={{ letterSpacing: '0.5px', opacity: 0.85, color: 'white', mx: 1 }}>
												VIBE HOUSE LLC
											</Typography>
										</Box>
										<Box sx={{ position: 'absolute', left: 0, top: '50%', width: '100%', height: 1, borderBottom: '2px dotted rgba(255, 255, 255, 0.1)', transform: 'translateY(-50%)' }} />
									</Box>
									<Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1.5, textAlign: 'center', opacity: 0.7 }}>
										Processed securely through Stripe
									</Typography>
								</Box>
							</Box>
							<Box sx={{ mx: 'auto', mt: 'auto', textAlign: 'center' }}>
								<LoadingButton loading={loading || !checkoutLink} loadingIndicator={<Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}><CircularProgress color="inherit" size={16} sx={{ mr: 1 }} /><span>Preparing&nbsp;Checkout...</span></Box>} variant="contained" size="large" fullWidth sx={{ borderRadius: 50, px: 0, py: 1.5, fontSize: 20, backgroundColor: getColor(), color: getTextColor() }} onClick={() => { window.location.href = checkoutLink; }}>
								...
								<Typography variant="caption" sx={{ mt: 2, lineHeight: 1.2 }}>
									...
									<a href="/termsofservice" target="_blank" style={{ color: 'white', textDecoration: 'none', fontWeight: 'bold' }}>Terms&nbsp;of&nbsp;Service</a>{' '}and{' '}
									<a href="/privacypolicy" target="_blank" style={{ color: 'white', textDecoration: 'none', fontWeight: 'bold' }}>Privacy&nbsp;Policy</a>.
								</Typography>
							</Box>
						</Box>
					)}
