...
export default function FramePage({ shows = [] }) {
  ...
  async function loopThroughKeys(obj, fps, subtitleObj) {
    ...
    const fileList = Object.keys(obj).map(async (key) => {
      const videoUrl = `http://ipfs.davis.pub/ipfs/${params.cid}/${subtitleObj.season}/${subtitleObj.episode}/${key}`
  ...
  useEffect(() => {
    if (showObj) {
      ...
      // http://ipfs.davis.pub/ipfs/${params.cid}/${result.season}/${result.episode}/s${parseInt(result.subtitle_index, 10)+1}.mp4
    }
